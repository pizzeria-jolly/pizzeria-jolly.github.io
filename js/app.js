...
function contattiClick() {
  ...
  if (fb.classList[1] == 'Hidden') {
    ...
    click.to(mainConteiner,1,{width:'calc(100% - 372px)'})
          .fromTo(fb,1,{display:'block',width:0},{width:'340px'});
    contattiFbShow();
  } else {
    contattiFbShow();
  };
...
function listinoClick() {
  ...
  if (boh.classList[1] == 'seeingListino') {
    for (var i = 0; i < 3; i++) {
      click.to(wrong,0.1,{x:'-5px'})
      .to(wrong,0.05,{x:'5px'})
      .to(wrong,0.1,{x:0});
